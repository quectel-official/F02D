 ...
 ****************************************************************************************
 */
#ifndef _COMMON_BUF_H_
#define _COMMON_BUF_H_

/**
 ****************************************************************************************
 * @defgroup COMMON_BUF Utilities
 * @ingroup COMMON
...
#include "architect.h"         // Arch defines
#include "common_list.h"      // List manipulation
#include "common_utils.h"     // Bit Field manipulation
...
#define COMMON_BUF_META_DATA_SIZE (32 >> 2)

/// Buffer Error status codes
enum common_buf_err
{
    /// No Error
    COMMON_BUF_ERR_NO_ERROR             = 0x00,
    /// Invalid parameter(s)
    COMMON_BUF_ERR_INVALID_PARAM        = 0x01,
    /// Not enough resources
    COMMON_BUF_ERR_INSUFFICIENT_RESOURCE = 0x02,
    /// Resource is busy, operation cannot be performed
    COMMON_BUF_ERR_RESOURCE_BUSY        = 0x03,
};


/// Buffer meta-data bit field
enum common_buf_metadata_bf
{
    /// Size of meta-data data frozen. This size has a step of 4 bytes
    COMMON_BUF_METADATA_FROZEN_SIZE_LSB                  = 0,
    COMMON_BUF_METADATA_FROZEN_SIZE_MASK                 = 0x0F,
    /// If equals 1, a callback is executed before freeing the buffer.
    COMMON_BUF_METADATA_FREE_CB_POS                      = 4,
    COMMON_BUF_METADATA_FREE_CB_BIT                      = 0x10,
};
 ...
 ****************************************************************************************
 */
typedef void (*common_buf_free_cb)(common_buf_t* p_buf, void* p_env);
 ...
 *
 * If total length is lower than @see COMMON_BUF_SMALL_SIZE and small buffer pool is not empty:
 *      A buffer will be picked from small buffer pool.
 * else if total length is lower than @see COMMON_BUF_BIG_SIZE and big buffer pool is not empty:
 ...
 *
 * @return COMMON_BUF_ERR_NO_ERROR if buffer can be allocated.
 *         COMMON_BUF_ERR_INSUFFICIENT_RESOURCE if no more buffers are available.
 ****************************************************************************************
 */
uint8_t common_buf_alloc(common_buf_t** pp_buf, uint16_t head_len, uint16_t data_len, uint16_t tail_len);
 ...
 * When doing a buffer allocation, acquisition counter is equals to 1.
 * Buffer isn’t free by @see common_buf_release function when acquisition counter is equals to 0.
 ...
 ****************************************************************************************
 */
uint8_t common_buf_prepare(common_buf_t* p_buf, uint16_t head_len, uint16_t data_len, uint16_t tail_len);
 ...
 *
 * @return COMMON_BUF_ERR_NO_ERROR if operation succeed
 ****************************************************************************************
 */
uint8_t common_buf_acquire(common_buf_t *p_buf);
 ...
 *            if a free callback has been configured using @see common_buf_cb_set:
 ...
 *
 *            else if a buffer has been initialized with @see common_buf_prepare:
 *                  nothing is done
 *
 ...
 * @param[in] p_buf        Pointer to acquired buffer.
 *
 * @return COMMON_BUF_ERR_NO_ERROR if buffer has been released.
 *         COMMON_BUF_ERR_INVALID_PARAM if buffer was free.
 ****************************************************************************************
 */
uint8_t common_buf_release(common_buf_t *p_buf);
 ...
 ****************************************************************************************
 */
__INLINE uint8_t* common_buf_data(common_buf_t *p_buf)
{
    uint8_t* p_ret = NULL;
 ...
 ****************************************************************************************
 */
__INLINE uint16_t common_buf_data_len(const common_buf_t *p_buf)
{
    uint16_t ret = 0;
 ...
 ****************************************************************************************
 */
__INLINE uint16_t common_buf_head_len(const common_buf_t *p_buf)
{
    uint16_t ret = 0;
 ...
 ****************************************************************************************
 */
uint16_t common_buf_size(const common_buf_t *p_buf);
 ...
 ****************************************************************************************
 */
__INLINE uint8_t* common_buf_head(common_buf_t *p_buf)
{
    uint8_t* p_ret = NULL;
 ...
 *
 * @return COMMON_BUF_ERR_NO_ERROR if needed length has been reserved.
 *         COMMON_BUF_ERR_INVALID_PARAM if provided length is higher than current length of head part.
 ****************************************************************************************
 */
uint8_t common_buf_head_reserve(common_buf_t *p_buf, uint16_t length);
 ...
 *
 * @return COMMON_BUF_ERR_NO_ERROR if needed length has been released.
 *         COMMON_BUF_ERR_INVALID_PARAM if provided length is higher than current length of data part.
 ****************************************************************************************
 */
uint8_t common_buf_head_release(common_buf_t *p_buf, uint16_t length);
 ...
 ****************************************************************************************
 */
__INLINE uint8_t* common_buf_tail(common_buf_t *p_buf)
{
    uint8_t* p_ret = NULL;
 ...
 ****************************************************************************************
 */
__INLINE uint16_t common_buf_tail_len(const common_buf_t *p_buf)
{
    uint16_t ret = 0;
 ...
 *
 * @return COMMON_BUF_ERR_NO_ERROR if needed length has been reserved.
 *         COMMON_BUF_ERR_INVALID_PARAM if provided length is higher than current length of tail part.
 ****************************************************************************************
 */
uint8_t common_buf_tail_reserve(common_buf_t *p_buf, uint16_t length);
 ...
 ****************************************************************************************
 */
uint8_t common_buf_tail_release(common_buf_t *p_buf, uint16_t length);
 ...
 ****************************************************************************************
 */
__INLINE uint8_t* common_buf_metadata(common_buf_t *p_buf)
{
    uint8_t* p_ret = NULL;

    if((p_buf) && (GETF(p_buf->metadata_bf, COMMON_BUF_METADATA_FROZEN_SIZE) < COMMON_BUF_META_DATA_SIZE))
    {
        p_ret = (uint8_t*) &(p_buf->metadata[GETF(p_buf->metadata_bf, COMMON_BUF_METADATA_FROZEN_SIZE)]);
    }
 ...
 *        pointer given by @see common_buf_metadata function .
 ...
 *
 * @return COMMON_BUF_ERR_NO_ERROR if needed length has been frozen.
 *         COMMON_BUF_ERR_INVALID_PARAM if provided length is higher than remaining meta-data size
 ****************************************************************************************
 */
uint8_t common_buf_metadata_freeze(common_buf_t *p_buf, uint8_t length);
 ...
 *
 * @return COMMON_BUF_ERR_NO_ERROR if needed length has been frozen.
 *         COMMON_BUF_ERR_INVALID_PARAM if provided length is higher than frozen meta-data size
 ****************************************************************************************
 */
uint8_t common_buf_metadata_unfreeze(common_buf_t *p_buf, uint8_t length);
 ...
 ****************************************************************************************
 */
__INLINE uint8_t common_buf_metadata_len(const common_buf_t *p_buf)
{
    uint8_t ret = 0;

    if(p_buf)
    {
        ret = (COMMON_BUF_META_DATA_SIZE - GETF(p_buf->metadata_bf, COMMON_BUF_METADATA_FROZEN_SIZE)) << 2;
    }
 ...
 ****************************************************************************************
 */
uint8_t common_buf_duplicate(const common_buf_t *p_buf_in, common_buf_t **pp_buf_out, uint16_t head_len, uint16_t tail_len);
 ...
 *
 * @return COMMON_BUF_ERR_NO_ERROR if copy has been properly performed.
 *         COMMON_BUF_ERR_INVALID_PARAM if the output buffer data size is cannot accept input data length.
 ****************************************************************************************
 */
uint8_t common_buf_copy(const common_buf_t *p_buf_in, common_buf_t *p_buf_out, uint16_t length, uint8_t copy_meta_size);
 ...
 *
 * @return COMMON_BUF_ERR_NO_ERROR if buffer has been properly released and reused
 *         COMMON_BUF_ERR_RESOURCE_BUSY if buffer acquisition counter > 1
 ****************************************************************************************
 */
uint8_t common_buf_reuse(common_buf_t *p_buf);
 ...
 * @return COMMON_BUF_ERR_NO_ERROR if buffer has been properly released and reused
 *         COMMON_BUF_ERR_RESOURCE_BUSY if buffer acquisition counter > 1
 *         COMMON_BUF_ERR_INVALID_PARAM if length fields exceed length of initial buffer
 ****************************************************************************************
 */
uint8_t common_buf_reuse_full(common_buf_t *p_buf, uint16_t head_len, uint16_t data_len, uint16_t tail_len);
 ...
 * @param[in] p_env        Pointer to environment that will be used as callback parameter.
 *
 * @return COMMON_BUF_ERR_NO_ERROR callback has been properly set
 *         COMMON_BUF_ERR_RESOURCE_BUSY if some buffer meta-data already frozen
 ****************************************************************************************
 */
uint8_t common_buf_cb_free_set(common_buf_t *p_buf, common_buf_free_cb cb_free, void* p_env);
 ...
 *
 * @return COMMON_BUF_ERR_NO_ERROR if copy has been properly performed.
 *         COMMON_BUF_ERR_INVALID_PARAM if data length fields < length parameter
 ****************************************************************************************
 */
uint8_t common_buf_copy_data_from_mem(common_buf_t *p_buf, const uint8_t *p_in, uint16_t length);
 ...
 ****************************************************************************************
 */
uint8_t common_buf_copy_data_to_mem(const common_buf_t *p_buf, uint8_t *p_out, uint16_t length);
 ...
 ****************************************************************************************
 */
void common_buf_init(uint8_t init_type, uint32_t* p_big_pool, uint32_t* p_small_pool);

/// @} COMMON_BUF

#endif // _COMMON_BUF_H_
