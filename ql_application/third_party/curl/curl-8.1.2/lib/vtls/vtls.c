 ...
 */
  #if !defined(QL_CSDK_SUPPORT)&&!defined(QL_AT_SUPPORT)
                               ...
                               bool blocking, bool *done)
{
  ...
  cft_connect = cf->next->cft->connect;
  result = cft_connect(cf->next, data, blocking, done);
  if(result || !*done)
    goto out;
