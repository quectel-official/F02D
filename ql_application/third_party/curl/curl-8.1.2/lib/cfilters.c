                       ...
                       size_t len, CURLcode *code)
{
  ...
  if(cf) {
    Curl_cft_recv *cft_recv = NULL;
    cft_recv = cf->cft->do_recv;
    return cft_recv(cf, data, buf, len, code);
  }
                       ...
                       const void *mem, size_t len, CURLcode *code)
{
  ...
  if(cf) {
    Curl_cft_send *cft_send = NULL;
    cft_send = cf->cft->do_send;
    return cft_send(cf, data, mem, len, code);
  }
...
{
  infof(data,"%s,cf[%p],blocking[%d]",__func__, cf, blocking);
  if(cf){
    Curl_cft_connect *cft_connect = NULL;
    cft_connect = cf->cft->connect;
    return cft_connect(cf, data, blocking, done);
  }
                           ...
                           bool *done)
{
  ...
  infof(data, "%s sockindex:%d, blocking:%d\n",__func__, sockindex, blocking);
  *done = cf->connected;
  if(!*done) {
    Curl_cft_connect  *cft_connect = NULL;
    cft_connect = cf->cft->connect;
    result = cft_connect(cf, data, blocking, done);
    if(!result && *done) {
