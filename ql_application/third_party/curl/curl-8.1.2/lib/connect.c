                         ...
                         bool duringconnect)
{
  ...
  infof(data, "%s, data->set.timeout[%d],nowp[%p],data->set.timeout[%d]\n",__func__, data->set.timeout, nowp, data->set.timeout);
  if(data->set.timeout > 0) {
    timeout_set = TIMEOUT_MAXTIME;
    maxtime_timeout_ms = data->set.timeout;
  }
  infof(data, "%s,duringconnect:%d", __func__, duringconnect);
  if(duringconnect) {
  ...
  if(timeout_set & TIMEOUT_CONNECT) {
    infof(data,"connect_timeout_ms:%ld",connect_timeout_ms);
    infof(data, "nowp:%d,%d\n", nowp->tv_sec, nowp->tv_usec);
    infof(data, "t_startsingle:%d,%d\n", data->progress.t_startsingle.tv_sec, data->progress.t_startsingle.tv_usec);
    infof(data, "diff:%d",(nowp->tv_sec - data->progress.t_startsingle.tv_sec));

    connect_timeout_ms -= Curl_timediff(*nowp, data->progress.t_startsingle);
    infof(data,"%s,connect_timeout_ms[%d]\n",__func__, connect_timeout_ms);
    if(!(timeout_set & TIMEOUT_MAXTIME) ||
       (connect_timeout_ms < maxtime_timeout_ms))
      timeout_ms = connect_timeout_ms;
  }
  infof(data, "%s, timeout_ms[%d]\n",__func__, timeout_ms);
  if(!timeout_ms)
                              ...
                              const struct Curl_dns_entry *remotehost)
{
  ...
  infof(data, "%s, timeout_ms[%ld]", __func__, timeout_ms);
   ...
   *
   */
  infof(data, "%s,ip_version[%d]",__func__, conn->ip_version);
                              ...
                              bool blocking, bool *done)
{
  ...
  DEBUGASSERT(ctx);
  *done = FALSE;
  infof(data,"%s,timestamp[%d]", __func__, ctx->remotehost->timestamp);
  switch(ctx->state) {
    case SCFST_INIT:
      DEBUGASSERT(CURL_SOCKET_BAD == Curl_conn_cf_get_socket(cf, data));
      DEBUGASSERT(!cf->connected);
      result = start_connect(cf, data, ctx->remotehost);
      infof(data,"%s,result[%d]", __func__, result);
      if(result)
        return result;
                        ...
                        struct Curl_easy *data)
{
  ...
  if(cf->next) {
    Curl_cft_close *cft_close = NULL;
    cft_close = cf->next->cft->close;
    cft_close(cf->next, data);
