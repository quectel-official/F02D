                                         ...
                                         int port)
{
  ...
  if(dns && (data->set.dns_cache_timeout != -1)) {
    ...
    //time(&user.now);
    user.now = ql_rtos_up_time_ms()/1000;
                ...
                int port)
{
  ...
  //time(&dns->timestamp);
  dns->timestamp = ql_rtos_up_time_ms()/1000;
  if(dns->timestamp == 0)
                           ...
                           struct Curl_dns_entry **entry)
{
  ...
  *entry = NULL;
  ...
  (void)allowDOH;
#endif
  infof(data,"%s, hostname[%s]",__func__, hostname);
  if(data->share)
    Curl_share_lock(data, CURL_LOCK_DATA_DNS, CURL_LOCK_ACCESS_SINGLE);
