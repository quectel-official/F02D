...
#include "dns.h"
extern int net_get_if_ipv6_addr(struct wlan_ip_config *addr, void *intrfc_handle);
static int  _mdns_get_ip6_linklocal(ip6_addr_t *ip6,  mdns_netif_t *mdns_netif)
{
	struct wlan_ip_config wlan_addr;

    memset(&wlan_addr, 0, sizeof(struct wlan_ip_config));
	   
	net_get_if_ipv6_addr(&wlan_addr, (void*)mdns_netif);

	for(int i = 0; i < MAX_IPV6_ADDRESSES; i++){
	    if(ip6_addr_islinklocal(&(wlan_addr.ipv6[i].address))){
	        memcpy(ip6, &wlan_addr.ipv6[i].address, sizeof(ip6_addr_t));
	    }
 ...
 *
 */
static mdns_netif_t * s_mdns_netifs[MDNS_IF_MAX] = {};
...
mdns_netif_t *_mdns_get_esp_netif(mdns_if_t tcpip_if)
{
    if (tcpip_if < MDNS_IF_MAX) {
        if (s_mdns_netifs[tcpip_if] == NULL) {
            // if local netif copy is NULL, try to search for the default interface key
            if (tcpip_if == MDNS_IF_STA) {
                s_mdns_netifs[MDNS_IF_STA] = net_get_sta_handle();
            } else if (tcpip_if == MDNS_IF_AP) {
                s_mdns_netifs[MDNS_IF_AP] = net_get_uap_handle();
#if CONFIG_ETH_ENABLED
            } else if (tcpip_if == MDNS_IF_ETH) {
                s_mdns_netifs[MDNS_IF_ETH] = NULL;
#endif
            }
        }
        return s_mdns_netifs[tcpip_if];
    }
    return NULL;
}
...
{
    struct netif *nif = net_get_if(mdns_netif);
#ifdef MDNS_ENABLE_DEBUG
     _mdns_dbg_printf("%s,nif:%p\n",__func__, nif);
#endif
    return nif;
}
/*
 ...
 */
#if 0
static mdns_if_t _mdns_get_if_from_esp_netif(mdns_netif_t *interface)
{
    for (int i=0; i<MDNS_IF_MAX; ++i) {
        if (interface == s_mdns_netifs[i])
            return i;
    }
    return MDNS_IF_MAX;
}
...
static uint8_t _mdns_append_answer(uint8_t * packet, uint16_t * index, mdns_out_answer_t * answer, mdns_if_t tcpip_if)
{
    if (answer->type == MDNS_TYPE_PTR) {
        ...
        struct wlan_ip_config  if_ip_info;
	    ...
	    memset((void *)&if_ip_info, 0x00, sizeof(struct wlan_ip_config));
        if (net_get_if_addr(&if_ip_info, _mdns_get_esp_netif(tcpip_if) )) {
            return 0;
        }
        if (_mdns_append_a_record(packet, index, if_ip_info.ipv4.address, answer->flush, answer->bye) <= 0) {
            return 0;
        }
        ...
        mdns_if_t other_if = _mdns_get_other_if (tcpip_if);
	    memset((void *)&if_ip_info, 0x00, sizeof(struct wlan_ip_config));
        if (net_get_if_addr(&if_ip_info, _mdns_get_esp_netif(other_if))) {
            return 1;
        }
        if (_mdns_append_a_record(packet, index, if_ip_info.ipv4.address, answer->flush, answer->bye) > 0) {
            return 2;
        ...
        if (_mdns_get_ip6_linklocal( &if_ip6, _mdns_get_esp_netif(tcpip_if))) {
            return 0;
        }
        ...
        mdns_if_t other_if = _mdns_get_other_if (tcpip_if);
        if (_mdns_get_ip6_linklocal(&if_ip6, _mdns_get_esp_netif(other_if))) {
            return 1;
        }
        if (_mdns_append_aaaa_record(packet, index, (uint8_t*)if_ip6.addr, answer->flush, answer->bye) > 0) {
            return 2;
...
static void _mdns_dispatch_tx_packet(mdns_tx_packet_t * p)
{
    ...
    if (p->dst.type == IPADDR_TYPE_V4) {
        _mdns_dbg_printf("To: " IPSTR ":%u, \n", IP2STR(&p->dst.u_addr.ip4), p->port);
    } else {
        _mdns_dbg_printf("To: " IPV6STR ":%u, \n", IPV62STR(p->dst.u_addr.ip6), p->port);
    }
...
static void _mdns_init_pcb_probe_new_service(mdns_if_t tcpip_if, mdns_ip_protocol_t ip_protocol, mdns_srv_item_t ** services, size_t len, bool probe_ip)
{
    ...
    _mdns_schedule_tx_packet(packet, ((pcb->failed_probes > 5)?1000:120) + (os_random() & 0x7F));
 ...
 */
static int _mdns_check_a_collision(ip4_addr_t * ip, mdns_if_t tcpip_if)
{
    struct wlan_ip_config  if_ip_info;
    struct wlan_ip_config  other_ip_info;
    if (!ip->addr) {
    ...
    int ret = memcmp((uint8_t*)&if_ip_info.ipv4.address, (uint8_t*)&ip->addr, sizeof(ip4_addr_t));
    if (ret > 0) {
        return -1;//we win
    } else if (ret < 0) {
        ...
        if (net_get_if_addr( &other_ip_info, _mdns_get_esp_netif(other_if))) {
            return 1;//IPv4 not active! They win
        }
        if (ip->addr != other_ip_info.ipv4.address) {
 ...
 */
static int _mdns_check_aaaa_collision(ip6_addr_t * ip, mdns_if_t tcpip_if)
{
    ip6_addr_t if_ip6;
    ip6_addr_t other_ip6;
    ...
    if (ret > 0) {
        ...
        if (_mdns_get_ip6_linklocal(&other_ip6, _mdns_get_esp_netif(other_if))) {
 ...
 */
static mdns_err_t _mdns_strdup_check(char ** out, char * in)
{
    if (in && in[0]) {
        *out = strdup(in);
        if (!*out) {
...
static int _mdns_netif_dhcpc_get_status(mdns_netif_t *tcpip_if)
{
	struct dhcp *dhcpc_ptr = netif_dhcp_data(net_get_if(tcpip_if));
    _mdns_dbg_printf("%s,dhcpc_ptr:%p\n",__func__, dhcpc_ptr);
    if(dhcpc_ptr){
        _mdns_dbg_printf("%s,dhcpc state:%d\n",__func__, dhcpc_ptr->state);
    }
	if(dhcpc_ptr == NULL || dhcpc_ptr->state == DHCP_STATE_OFF){
		return MDNS_NETIF_DHCP_STOPPED;
	}
	if(dhcpc_ptr->state == DHCP_STATE_INIT){
		return MDNS_NETIF_DHCP_INIT;
	}

	return MDNS_NETIF_DHCP_STARTED;
}
#endif
/**
 * @brief  Dispatch interface changes based on system events
 */
static void _mdns_handle_system_event( int32_t event_id, mdns_netif_t* interface)
{
    if (!_mdns_server) {
        return;
    }
...
/*
      case RW_EVT_STA_CONNECTED:
                if (_mdns_netif_dhcpc_get_status(_mdns_get_esp_netif(MDNS_IF_STA)) == MDNS_NETIF_DHCP_STOPPED) {
    ...
    case ACTION_TX_HANDLE:
        {
            ...
            if (p && p==action->data.tx_handle.packet && p->queued) {
                ...
                bk_printf("Skipping transmit of an unexpected packet!");
            }
        }
        break;
    case ACTION_RX_HANDLE:
 ...
 */
static mdns_err_t _mdns_send_search_action(mdns_action_type_t type, mdns_search_once_t * search)
{
    mdns_action_t * action = NULL;
    ...
    while (s) {
        if (s->state != SEARCH_OFF) {
            if (now > (s->started_at + s->timeout)) {
                s->state = SEARCH_OFF;
                if (_mdns_send_search_action(ACTION_SEARCH_END, s) != MDNS_OK) {
                    s->state = SEARCH_RUNNING;
                }
            } else if (s->state == SEARCH_INIT || (now - s->sent_at) > 1000) {
                s->state = SEARCH_RUNNING;
                s->sent_at = now;
                if (_mdns_send_search_action(ACTION_SEARCH_SEND, s) != MDNS_OK) {
                    s->sent_at -= 1000;
                }
            }
        }
...
static mdns_err_t _mdns_start_timer(void){
   if(rtos_init_timer(&(_mdns_server->timer_handle), MDNS_TIMER_PERIOD_MS, _mdns_timer_cb, NULL) == kNoErr){
	  rtos_start_timer(&(_mdns_server->timer_handle));
	  return MDNS_OK;
   }
...
static mdns_err_t _mdns_stop_timer(void){
    mdns_err_t err = MDNS_OK;
    if (rtos_is_timer_init(&_mdns_server->timer_handle)) {
	 if(rtos_is_timer_running(&_mdns_server->timer_handle)){
		rtos_stop_timer(&_mdns_server->timer_handle);
	 }
        rtos_deinit_timer(&_mdns_server->timer_handle);        
    }
    return err;
}
 ...
 *      - ESP_FAIL on error
 */
static mdns_err_t _mdns_service_task_start(void)
{
    ...
    if (!_mdns_service_task_handle) {
        xTaskCreate(_mdns_service_task, "mdns", MDNS_SERVICE_STACK_DEPTH, NULL, MDNS_TASK_PRIORITY,
                                (TaskHandle_t * const)(&_mdns_service_task_handle));
        if (!_mdns_service_task_handle) {
            bk_printf("%s[%d]\n",__func__,__LINE__);
            _mdns_stop_timer();
            MDNS_SERVICE_UNLOCK();
 ...
 *      - ESP_OK
 */
static mdns_err_t _mdns_service_task_stop(void)
{
    _mdns_stop_timer();
    if (_mdns_service_task_handle) {
...
mdns_err_t mdns_init(void)
{
    ...
    memset(s_mdns_netifs, 0, sizeof(s_mdns_netifs));
    ...
    for (i=0; i<MDNS_IF_MAX; i++) {
        if (!_mdns_get_ip6_linklocal(&tmp_addr6, _mdns_get_esp_netif(i)) && !_ipv6_address_is_zero(tmp_addr6)) {
            _mdns_enable_pcb(i, MDNS_IP_PROTOCOL_V6);
        }
	    memset((void *)&if_ip_info, 0x00, sizeof(struct wlan_ip_config));
        if (!net_get_if_addr( &if_ip_info, _mdns_get_esp_netif(i)) && if_ip_info.ipv4.address) {
...
mdns_err_t mdns_instance_name_set(const char * instance)
{
    if (!_mdns_server) {
        return MDNS_ERR_INVALID_ARG;
    }
...
mdns_err_t mdns_service_add(const char * instance, const char * service, const char * proto, uint16_t port, mdns_txt_item_t txt[], size_t num_items)
{
...
mdns_err_t mdns_service_port_set(const char * service, const char * proto, uint16_t port)
{
...
mdns_err_t mdns_service_txt_set(const char * service, const char * proto, mdns_txt_item_t txt[], uint8_t num_items)
{
...
mdns_err_t mdns_service_txt_item_set(const char * service, const char * proto, const char * key, const char * value)
{
...
mdns_err_t mdns_service_txt_item_remove(const char * service, const char * proto, const char * key)
{
...
mdns_err_t mdns_service_instance_name_set(const char * service, const char * proto, const char * instance)
{
...
mdns_err_t mdns_service_remove(const char * service, const char * proto)
{
    if (!_mdns_server || !_mdns_server->services || _str_null_or_empty(service) || _str_null_or_empty(proto)) {
        return MDNS_ERR_INVALID_ARG;
    }
...
mdns_err_t mdns_query(const char * name, const char * service, const char * proto, uint16_t type, uint32_t timeout, size_t max_results, mdns_result_t ** results)
{
...
mdns_err_t mdns_query_ptr(const char * service, const char * proto, uint32_t timeout, size_t max_results, mdns_result_t ** results)
{
...
mdns_err_t mdns_query_srv(const char * instance, const char * service, const char * proto, uint32_t timeout, mdns_result_t ** result)
{
...
mdns_err_t mdns_query_txt(const char * instance, const char * service, const char * proto, uint32_t timeout, mdns_result_t ** result)
{
    if (_str_null_or_empty(instance) || _str_null_or_empty(service) || _str_null_or_empty(proto)) {
        return MDNS_ERR_INVALID_ARG;
    }
...
mdns_err_t mdns_query_a(const char * name, uint32_t timeout, ip4_addr_t * addr)
{
    mdns_result_t * result = NULL;
    mdns_err_t err;
...
mdns_err_t mdns_query_aaaa(const char * name, uint32_t timeout, ip6_addr_t * addr)
{
    mdns_result_t * result = NULL;
    mdns_err_t err;
